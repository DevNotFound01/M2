...
bool CAsyncSQL::Connect()
{
	...
	//sys_err("Connection SQL_PLAYER_4: %s %s %s %s %d", m_stHost.c_str(), m_stUser.c_str(), m_stPassword.c_str(), m_stDB.c_str(), m_iPort);
#ifdef __WIN32__
					  ...
					  const char * c_pszLocale, bool bNoThread, int iPort)
{
	...
	//sys_err("Connection SQL_PLAYER_3: %s %s", m_stHost.c_str(), c_pszHost);
